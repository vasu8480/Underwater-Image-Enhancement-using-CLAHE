clc;
close all;
filename = imread('C:\Users\vasu\Desktop\datasets\U45_test\23.png'); 
...
[dehazed_image,t,A]=imreducehaze(hazy_image,'method','approxdcp');
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%    DCP      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
imshowpair(RGB_Image_o,dehazed_image,'montage')
title('2.ori (left) and DCP(right) Image');
img2=im2double(dehazed_image);   
figure;imshowpair(RGB_Image_o,img2,'montage');title('2. ORIGINAL (left) and it''s color(right) Image');

img3=clahe(img2);
figure;imshowpair(RGB_Image_o,img2,'montage');title('3. ORIGINAL (left) and it''s CLAHE(right) Image');
img33=im2uint8(img3);
